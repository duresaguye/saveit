...
export default function SharedFolderPage() {
  ...
  const saveAllToMyLinks = () => {
    try {
      ...
      const newLinkIds: string[] = existingLinks.slice(-newLinksCount).map((link: Link) => link.id)
      ...
      toast(`Saved ${newLinksCount} links to a new folder: "${folderName}"`,
      )
    } catch (err) {
      toast("Failed to save links",
        )
    }
  }
