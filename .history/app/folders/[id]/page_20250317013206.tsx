...
export default function FolderDetailPage() {
  ...
  useEffect(() => {
    if (folder) {
      ...
      const updatedFolders: Folder[] = allFolders.map((f: Folder) => (f.id === folder.id ? folder : f))
  ...
  const handleRemoveLink = (linkId: string) => {
    if (folder) {
      ...
      const updatedFolder: Folder = {
        ...folder,
        links: folder.links.filter((id: string) => id !== linkId),
      }
      setFolder(updatedFolder)

      // Update folderLinks
      setFolderLinks(folderLinks.filter((link: Link) => link.id !== linkId))
  ...
  const handleAddNewLink = (newLink: NewLink) => {
    ...
    // Add to all links
    const updatedLinks: Link[] = [...links, linkWithId]
...
const MindMapView: React.FC<{ links: Link[], onRemove: (id: string) => void, onShare: (link: Link) => void }> = ({ links, onRemove, onShare }) => {
  ...
  ((source: Link, i: number) => {
    links.slice(i + 1).forEach((target: Link) => {
      const commonTags: string[] = source.tags.filter((tag: string) => target.tags.includes(tag))
      if (commonTags.length > 0) {
        connections.push({
          source: source.id,
          target: target.id,
