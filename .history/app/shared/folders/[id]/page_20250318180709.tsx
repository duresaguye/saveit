...
export default function SharedFoldersPage() {
  ...
  useEffect(() => {
    ...
    const fetchSharedFolders = async () => {
      try {
        ...
        const foundFolders = allFolders.filter((folder) => folderIds.includes(folder.id))
  ...
  const getFolderLinks = (folderId) => {
    const folder = folders.find((f) => f.id === folderId)
    if (!folder) return []

    return folder.links.map((linkId) => allLinks.find((link) => link.id === linkId)).filter(Boolean)
  }
    ...
    try {
      ...
      getAllFolderLinks().forEach((link) => {
        ...
        if (!exists) {
          ...
          const existingLink = existingLinks.find((existingLink) => existingLink.url === link.url)
          if (existingLink) {
      ...
      toast({
        title: "Success",
        description: `Saved ${newLinksCount} new links to ${folders.length} folders`,
      })
    } catch (err) {
      toast({
        title: "Error",
