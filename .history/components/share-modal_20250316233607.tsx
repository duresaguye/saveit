...
export default function ShareModal({ isOpen, onClose, link }) {
  ...
  const shareWithGroups = () => {
    ...
    toast({
      title: "Group sharing prepared",
      description: `Sharing with ${uniqueRecipients.length} recipients.`,
    })
  }
  ...
  const shareViaSocial = (platform) => {
    ...
    toast({
      title: "Sharing",
      description: `Opening ${platform} to share your link.`,
    })
  }
  ...
  const addNewGroup = () => {
    ...
    if (editingGroup) {
      ...
      toast({
        title: "Group updated",
        description: `"${newGroupName}" has been updated.`,
      })
    } else {
      // Add new group
      const newGroup = {
